...
class YoloNasDetectionModel(DetectionModel):
    def __init__(
        ...
        **kwargs,
        ...
        if not model_path:  # use pretrained models downloaded from Deci-AI remote client
            ...
            if isinstance(class_names_yaml_path, list):
                self.class_names = class_names_yaml_path
            else:
    ...
    def set_model(self, model: Any):
        ...
        if model.__class__.__module__.split(".")[-1] not in (
            "yolo_nas_variants",
            "yolox",
        ):
        ...
        processing_params["iou"] = self._yolo_iou_threshold
...
class SgYoloXDetectionModel(DetectionModel):
    def __init__(
        ...
        yolo_iou_threshold: Optional[float]=0.65,
        **kwargs,
    ):
        if model_name is not None and not isinstance(model_name, str):
            raise TypeError(
    ...
    def perform_inference(self, image: np.ndarray):
        ...
        if self.pipeline is None:
            raise ValueError("Model is not loaded, load it by calling .load_model()")
        prediction_result = [self.pipeline(image)]
        self._original_predictions = prediction_result
